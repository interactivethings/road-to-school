...
export function baseline(force, data, {width, height}) { 
  ...
  force
    .force('xSchool', isolate(data, d3.forceX(width/4), function(d) { return d.type === 'school' ; }))
    .force('ySchool', isolate(data, d3.forceY(height/2), function(d) { return d.type === 'school' ; }))
    .force('xNoSchool', isolate(data, d3.forceX(width/4), function(d) { return d.type === 'noSchool' ; }))      
...
export function outOfSchool(force, data, {width, height}) {
  // console.log('outOfSchool')
    ...
    force
        .force('xSchool', isolate(data, d3.forceX(width/4), function(d) { return d.type === 'school' ; }).strength(function(d,i) { return d.id/(data.length*8); }))
        ...
        .force('xNoSchool', isolate(data, d3.forceX(function(d,i) { return Math.sin(i) * R * getVariation(0.8,1) + (width/4);} ), function(d) { return d.type === 'noSchool'; }).strength(0.2))      
...
export function backToSchool(force, data, {width, height}) {
  ...
  force
    .force('xSchool', isolate(data, d3.forceX(width/4), function(d) { return d.type === 'school' ; }).strength(0.8))
    .force('ySchool', isolate(data, d3.forceY(height/2), function(d) { return d.type === 'school' ; }).strength(0.8))
    .force('xNoSchool', isolate(data, d3.forceX(width/4), function(d) { return d.type === 'noSchool' ; }).strength(0.8))      
...
export function bombForce(force, data, {width, height}) {
    ...
    //     bombForce.force('bombX', d3.forceX(function(d,i) { return Math.sin(i) *  R * getVariation(1,1.2) + width/4 * getVariation(0.9,1.2);} ).strength(-0.1))
