...
export function outOfSchool(force, data, {width, height}) {
    ...
    force
        ...
        .force('ySchool', isolate(data, d3.forceY(height*0.8/2), function(d) { return d.type === 'school' ; })
            .strength(0.2))
        .force('xNoSchool', isolate(data, d3.forceX(function(d,i) { return Math.sin(i) * R * getVariation(0.9,1) + (width*0.7/2);} ), function(d,i) { return d.type === 'noSchool'; }).strength(0.2))      
        .force('yNoSchool', isolate(data, d3.forceY(function(d,i) { return Math.cos(i) * R * getVariation(0.9,1) + (height*0.8/2);} ), function(d) { return d.type === 'noSchool'; }).strength(0.2))  
        .force('collideSchool', isolate(data, d3.forceCollide(), function(d) { return d.type === 'school' ; }).radius(function(d) { return d.r * 2; }).iterations(2).strength(0.8)) 
